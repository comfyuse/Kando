...
// Components - Keep imports for structure but they will show coming soon state
// import ProfilePanel from './components/ProfilePanel';
// import ChatPanel from './components/ChatPanel';
// import MessageRequests from './components/MessageRequests';
// import FriendsList from './components/FriendsList';

// Data - Keep for structure
import { citizenNames, personProfiles, generatePeerHash, UserProfile, MessageRequest } from './data/profiles';

// Import Scene2D - will be hidden in coming soon mode
const Scene2D = dynamic(() => import('@/components/Scene2D'), { ssr: false });
...
// Dummy Cell class for compatibility
class Cell {
  coord: any;
  status: string;
  isAlive: () => boolean;
...
export default function ChatPage() {
  // ============================================
  // COMING SOON MODE - All P2P functionality is disabled
  ...
  const ComingSoonScreen = () => (
    <div className="w-full h-screen bg-gradient-to-br from-[#0a0a0f] via-[#0d1117] to-[#0a0a0f] overflow-hidden relative">
      {/* Animated Background */}
      <div className="absolute inset-0 bg-gradient-to-br from-[var(--jade)]/5 via-transparent to-[var(--jade)]/5 animate-pulse-slow pointer-events-none" />
      <div className="absolute top-0 left-0 w-96 h-96 bg-[var(--jade)]/10 rounded-full blur-3xl animate-float pointer-events-none" />
      <div className="absolute bottom-0 right-0 w-96 h-96 bg-[var(--jade)]/10 rounded-full blur-3xl animate-float-delayed pointer-events-none" />
      ...
      <header className="absolute top-0 left-0 right-0 z-20 pointer-events-auto">
        ...
        {mobileMenuOpen && isMobile && (
          <div className="absolute top-full left-0 right-0 bg-[var(--bg-secondary)]/95 backdrop-blur-xl border-b border-white/10 p-4 flex flex-col gap-3 animate-slideDown z-30">
            <div className="flex items-center justify-between p-2 rounded-lg bg-white/5">
              <span className="text-sm text-[var(--text-primary)]">P2P Chat Status</span>
              <div className="flex items-center gap-2">
      ...
      <div className="absolute top-16 md:top-20 left-0 right-0 z-20 pointer-events-auto animate-fadeIn px-2 md:px-0" style={{ animationDelay: '0.15s' }}>
        <div className="flex justify-center">
          <div className="glass-modern px-2 md:px-4 py-1.5 md:py-2 rounded-xl md:rounded-2xl flex gap-1 md:gap-2 overflow-x-auto scrollbar-hide max-w-[calc(100vw-1rem)] md:max-w-none">
            <div className="stat-card-modern px-3 md:px-4 py-1 md:py-1.5 text-center">
              <div className="text-[7px] md:text-[8px] font-semibold text-[var(--text-muted)] tracking-wider uppercase">STATUS</div>
              <div className="text-[10px] md:text-sm font-bold tabular-nums text-yellow-400">COMING SOON</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute inset-0 flex items-center justify-center z-20 px-4">
        ...
        >
          ...
          <motion.div
            ...
            className="w-24 h-24 mx-auto mb-6 rounded-2xl bg-gradient-to-br from-[var(--jade)]/20 to-[var(--jade)]/5 flex items-center justify-center"
          >
            <svg className="w-12 h-12 text-[var(--jade)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
            </svg>
          </motion.div>
          
          <h2 className="text-2xl md:text-4xl font-bold text-[var(--text-primary)] mb-3">
            P2P Chat Coming Soon
          </h2>
          
          <p className="text-sm md:text-base text-[var(--text-muted)] mb-6 max-w-md mx-auto">
            We're building a decentralized peer-to-peer messaging system with DHT-based peer discovery and end-to-end encryption.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-8 text-left">
            {[
              ...
              { icon: '🌐', text: 'Decentralized P2P network' },
              { icon: '🔑', text: 'DHT-based peer discovery' },
              { icon: '💬', text: 'Real-time messaging' },
              { icon: '👥', text: 'Friend system' },
              { icon: '📨', text: 'Message requests' }
            ].map((feature, idx) => (
              <motion.div
                key={idx}
              ...
              >
                ...
                <span className="text-xs md:text-sm text-[var(--text-secondary)]">{feature.text}</span>
              </motion.div>
            ))}
          </div>
          ...
          <div className="max-w-md mx-auto mb-6">
            ...
            <div className="w-full h-2 bg-white/10 rounded-full overflow-hidden">
              <motion.div
                ...
                className="h-full bg-gradient-to-r from-[var(--jade)] to-[var(--jade-hover)] rounded-full"
              />
            </div>
          </div>
          ...
          <Link href="/">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-[var(--jade)] to-[var(--jade-hover)] text-white font-medium shadow-lg shadow-[var(--jade)]/20 hover:opacity-90 transition-all"
            >
              Back to Home
            </motion.button>
          </Link>
      ...
      <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 pointer-events-auto animate-fadeIn opacity-50" style={{ animationDelay: '0.2s' }}>
        <div className="glass-modern flex gap-2 p-1 rounded-2xl">
          <button className="px-5 py-2 text-xs font-medium rounded-xl bg-white/5 text-[var(--text-muted)] cursor-not-allowed">
            STEP
          </button>
          <button className="px-5 py-2 text-xs font-medium rounded-xl bg-white/5 text-[var(--text-muted)] cursor-not-allowed">
            AUTO
